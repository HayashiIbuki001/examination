...
public class MoveSquareEnemy : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            case 0: if (offset.x >= moveDistanceX) { NextDirection(); } break; // right
            case 1: if (offset.y >= moveDistanceY) { NextDirection(); } break; // up
            case 2: if (offset.x <= 0f)            { NextDirection(); } break; // left
            case 3: if (offset.y <= 0f)            { NextDirection(); } break; // down
        }
